...
def load_shutdown_history(file_path: str, modified_time: float) -> pd.DataFrame:
    """
    读取停机历史。模板字段：
    machine / stop_start / stop_end / remark / load_mw_override(可选)
    同时兼容常见中文列名。
    """
    _ = modified_time
...
def build_shutdown_period_summary(
    ...
    merged = machine_info.merge(
        analysis_detail,
        on=["日期", "时段"],
        how="left",
    )

    # 实际卖出量使用各阶段负调整之和，避免“前面买、实时大卖”被最终净额抵消。
    merged["观察到的阶段卖出_MWh"] = (
    ...
    # 停机相关价格影响：只把理论停机量与实际卖出量较小者归因给停机。
    # 正值 = 相对LT基准低价回售增加成本；负值 = 回售价高于LT基准带来有利结果。
    merged["停机相关价格影响估算_元"] = np.where(
        merged["停机可解释卖出_MWh"].gt(1e-9)
        & merged["LT加权均价"].notna()
        & merged["回售等效均价"].notna(),
        merged["停机可解释卖出_MWh"]
        * (merged["LT加权均价"] - merged["回售等效均价"]),
        0.0,
    )
    return merged
...
def calculate_shutdown_period_impact(
    ...
    """
    汇总峰/平/谷：实际综合均价、停机相关价格影响、估算剔除停机后的均价。
    这是基于PM典型MW和交易回售价格的归因估算，不是正式结算重算。
    """
    ...
    period_net = base.groupby("峰平谷")["energy_mwh"].sum().to_dict()

    if shutdown_period_summary is None or shutdown_period_summary.empty:
        shutdown_impact = {}
    else:
        shutdown_impact = (
            shutdown_period_summary.groupby("时段")["停机相关价格影响估算_元"].sum().to_dict()
        )

    rows = []
    for p in ["峰", "平", "谷"]:
        actual = actual_prices.get(p, np.nan)
        net_energy = float(period_net.get(p, 0.0))
        impact_yuan = float(shutdown_impact.get(p, 0.0))
        uplift = impact_yuan / net_energy if abs(net_energy) > 1e-9 else np.nan
        normalized = actual - uplift if pd.notna(actual) and pd.notna(uplift) else np.nan
        rows.append({
            "时段": p,
            "实际综合均价_元每MWh": actual,
            "最终净电量_MWh": net_energy,
            "停机相关价格影响估算_元": impact_yuan,
            "停机对均价影响估算_元每MWh": uplift,
            "剔除已知停机影响后估算均价_元每MWh": normalized,
        })
    return pd.DataFrame(rows)
       ...
       if not (
           ...
           if not shutdown_period_summary.empty:
               ...
               total_shutdown_cost = float(
                   shutdown_period_summary["停机相关价格影响估算_元"].sum()
               )
               ...
               summary_lines.append(
                   ...
                   f"与实际市场卖出匹配后，停机相关价格影响估算合计约 "
                   f"**{total_shutdown_cost:+,.0f} 元**。正值表示低价回售相对LT基准增加成本。"
               )
               ...
               if not shutdown_period_impact.empty:
                   ...
                   if pd.notna(top_impact["停机对均价影响估算_元每MWh"]):
                       summary_lines.append(
                           ...
                           f"剔除已知停机价格影响后的估算均价约 "
                           f"{top_impact['剔除已知停机影响后估算均价_元每MWh']:.2f} 元/MWh。"
                       )

               top_shutdown = shutdown_period_summary.sort_values(
                   "停机相关价格影响估算_元", ascending=False
               ).head(3)
           ...
           st.caption(
               ...
               "停机Excel可用load_mw_override为单次记录覆盖默认值。"
               "停机开始/结束支持精确到分钟和跨天，代码按实际重叠分钟拆入峰/平/谷。"
           )
           ...
           if shutdown_period_impact.empty:
               ...
               imp = shutdown_period_impact.rename(columns={
                   ...
                   "最终净电量_MWh": "最终净电量(MWh)",
                   "停机相关价格影响估算_元": "停机相关价格影响估算(元)",
                   "停机对均价影响估算_元每MWh": "停机对均价影响估算(元/MWh)",
                   "剔除已知停机影响后估算均价_元每MWh": "剔除已知停机影响后估算均价(元/MWh)",
               })
               st.dataframe(imp.round(2), use_container_width=True, hide_index=True)
               st.caption(
                   "“剔除停机影响后估算均价”=实际综合均价−停机相关价格影响/该时段最终净电量。"
                   "这是基于纸机典型MW、已知停机分钟和实际回售价格的反事实近似，用于解释均价，不替代正式结算。"
               )
           ...
           if shutdown_period_summary.empty:
               ...
               for _, r in shutdown_sorted.iterrows():
                   ...
                   with st.expander(title, expanded=True):
                       ...
                       impact = float(r.get("停机相关价格影响估算_元", 0) or 0)
                       if abs(impact) > 1e-6:
                           direction = "增加成本" if impact > 0 else "形成有利价格结果"
                           st.caption(
                               f"该停机时段相关价格影响估算 {impact:+,.0f} 元（{direction}）。"
                           )
           ...
           st.caption(
               "只评价已知停机/明显负荷下降之外的交易结果。单次达到阈值仅称为‘偶发不利结果’，"
...
默认PM1=31 MW、PM2=28 MW、PM3=23 MW，可在代码配置区调整，或在Excel单条记录中用`load_mw_override`覆盖。

**3. 停机可解释卖出量**  
取`理论停机负荷下降`与`能量块/日前/实时各阶段实际负调整之和`的较小值。这样既不把超过理论停机负荷的卖出全部归因于停机，也不会因为前序买入抵消实时大卖而漏判。

**4. 停机相关价格影响**  
`停机可解释卖出量 × (LT加权均价 − 实际回售等效均价)`。正值表示相对LT基准低价回售增加成本；负值表示回售价高于LT基准。  
`停机对均价影响 ≈ 停机相关价格影响 ÷ 对应峰/平/谷最终净电量`。
