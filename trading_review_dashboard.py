...
def deviation_recovery_summary(df: pd.DataFrame) -> tuple[float, float]:
  """返回日前偏差费用总额和偏差电量。"""
  if df.empty:
      return 0.0, 0.0
  ...
  if "deviation_recovery_yuan" in df.columns:
      fee = float(
          safe_numeric(df["deviation_recovery_yuan"]).fillna(0).sum()
      )
...
# 日前、实时市场没有绿电和环境权益。
# 在数值清洗完成后生成现货计算数据源。
deviation_fee, deviation_energy = deviation_recovery_summary(
  deviation_raw
)
...
with st.container():
  ...
  with d1:
      render_kpi(
          "日前偏差费用",
          f"{deviation_fee:,.2f} 元",
          "增加成本",
      )
  with d2:
      actual_cost = total_amount + deviation_fee
      render_kpi(
          "实际结算成本",
          f"{actual_cost:,.2f} 元",
          "交易费用 + 日前偏差费用",
