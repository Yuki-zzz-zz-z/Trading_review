...
def _infer_load_change_stage(lt, eb, da, rt, params):
    """
    根据能量块→日前→实时的净调整路径，推测负荷下降信息大致何时反映到交易安排。

    这里只解释“交易侧何时开始反映负荷下修”，不依赖停机类型标签，也不直接认定
    计划/非计划。最小显著信号沿用正常调整上限（默认10% LT）。
    """
    if lt <= 1e-9:
    ...
    eb_sig = eb_sell >= min_signal
    da_sig = da_sell >= min_signal
    rt_sig = rt_sell >= min_signal
    rt_small = abs(rt) < min_signal

    # 典型的“较早已知”：能量块已经明显卖出，日前继续调整，而实时只剩小幅纠偏。
    if eb_sig and da_sig and rt_small:
        return (
            "较早已知 / 分阶段调整",
            "能量块阶段已开始明显降低已购电量，日前继续卖出，而实时仅小幅调整；"
            "说明主要负荷下修信息在实时之前已经进入交易安排，更符合提前获知后为控制风险分阶段调整交易电量。",
        )

    # 能量块已经出现明显下修信号，但实际下降幅度在后续继续扩大。
    if eb_sig and rt_sig:
        return (
            "较早已有下修信号，实时继续扩大",
            "能量块阶段已经出现明显卖出，说明较早已有负荷下修信号；实时仍继续大幅卖出，"
            "表明后续实际负荷下降幅度进一步扩大，可能与停机持续时间、恢复时间或实际运行负荷继续变化有关。",
        )

    # 能量块阶段尚未明显降低已购电量，日前开始完成主要下修。
    if not eb_sig and da_sig and not rt_sig:
        return (
            "能量块后明确 / 日前主要调整",
            "能量块阶段未出现明显卖出，日前阶段开始大幅降低已购电量，实时仅剩小幅调整；"
            "说明负荷下降信息更可能在能量块交易之后、日前交易阶段前后变得明确。",
        )

    if not eb_sig and da_sig and rt_sig:
        return (
            "日前开始下修，实时继续扩大",
            "能量块阶段未明显卖出，日前已经开始降低已购电量，但实时仍继续大幅卖出；"
            "说明负荷下修信息在日前阶段已出现，但最终实际下降幅度仍高于此前判断。",
        )

    # 最晚发现型：前两个阶段没有明显卖出，实时突然大幅下修。
    if not eb_sig and not da_sig and rt_sig:
        return (
            "临近运行才明显下修",
            "能量块和日前阶段均未出现明显卖出，而实时阶段突然大幅降低已购电量；"
            "说明主要负荷下降直到临近实际运行时才反映到交易结果，是较晚获知或突发负荷下降的典型路径。",
        )

    if eb_sig and rt_small:
        return (
            "能量块阶段已提前反映",
            "能量块阶段已经明显降低已购电量，后续尤其是实时调整较小，说明负荷下降较早已进入交易安排。",
        )

    if da_sig:
        return (
            "日前阶段主要反映",
            "主要负调整集中在日前阶段，说明负荷下降信息在日前交易阶段已经较明显。",
        )

    if rt_sig:
        return (
            "实时阶段主要下修",
            "主要负调整集中在实时阶段，说明负荷变化较晚才明显反映到交易安排。",
        )
...
def calculate_normal_purchase_structure(
    df: pd.DataFrame,
    ...
    """
    统计“正常运行日”在能量块 / 日前 / 实时三个后续市场中的正向购电结构。

    正常样本排除：
    1) 已知PM停机日期-时段；
    2) 交易链已显示明显负荷下降/实时大幅纠偏的日期-时段。

    比例只用于无停机反事实中“撤销实际回售后仍不足的正常需求”的购电分配，
    不代表真实交易规则，也不用于倒推最优市场。
    """
    cols = ["时段", "样本天数", "能量块购电_MWh", "日前购电_MWh", "实时购电_MWh",
            "能量块占比", "日前占比", "实时占比", "样本口径"]
    ...
    out["display_date"] = pd.to_datetime(out["display_date"], errors="coerce").dt.normalize()
    out = out.dropna(subset=["datetime_bj", "display_date"]).copy()
    out["时段"] = out["datetime_bj"].apply(period_type)
    out["energy_mwh"] = safe_numeric(out["energy_mwh"]).fillna(0)

    blocked = set()
    if shutdown_split is not None and not shutdown_split.empty:
        blocked |= set(
            (pd.Timestamp(r["日期"]).normalize(), str(r["时段"]))
            for _, r in shutdown_split.iterrows()
        )
    if analysis_detail is not None and not analysis_detail.empty:
        bad = analysis_detail[
            analysis_detail["负荷下降信号率"].ge(params["resale_general_max"])
        ]
        blocked |= set(
            (pd.Timestamp(r["日期"]).normalize(), str(r["时段"]))
    ...
    key_series = list(zip(out["display_date"], out["时段"].astype(str)))
    out = out[[k not in blocked for k in key_series]].copy()
    out = out[out["market_stage"].isin(["energy_block", "day_ahead", "real_time"])].copy()
    out = out[out["energy_mwh"] > 1e-9].copy()

    def summarize(source: pd.DataFrame, period: str, label: str):
        g = source if period == "全部" else source[source["时段"].eq(period)]
        vals = {
            "energy_block": float(g.loc[g["market_stage"].eq("energy_block"), "energy_mwh"].sum()),
            "day_ahead": float(g.loc[g["market_stage"].eq("day_ahead"), "energy_mwh"].sum()),
            "real_time": float(g.loc[g["market_stage"].eq("real_time"), "energy_mwh"].sum()),
        }
        ...
        return {
            "时段": period,
            "样本天数": int(g["display_date"].nunique()),
            "能量块购电_MWh": vals["energy_block"],
            "日前购电_MWh": vals["day_ahead"],
            "实时购电_MWh": vals["real_time"],
            "能量块占比": vals["energy_block"] / total,
            "日前占比": vals["day_ahead"] / total,
            "实时占比": vals["real_time"] / total,
            "样本口径": label,
        }

    rows = []
    overall = summarize(out, "全部", "正常运行样本（排除已知停机及明显负荷下降）")
    if overall is not None:
        rows.append(overall)
    for period in ["峰", "平", "谷"]:
        r = summarize(out, period, "正常运行样本（排除已知停机及明显负荷下降）")
        if r is not None:
            rows.append(r)

    # 若严格正常样本为空，退回全部有效交易日的正向购电结构，但明确标注。
    if not rows:
        fallback = df.copy()
        fallback["datetime_bj"] = pd.to_datetime(fallback["datetime_bj"], errors="coerce")
        fallback["display_date"] = pd.to_datetime(fallback["display_date"], errors="coerce").dt.normalize()
        fallback = fallback.dropna(subset=["datetime_bj", "display_date"]).copy()
        fallback["时段"] = fallback["datetime_bj"].apply(period_type)
        fallback["energy_mwh"] = safe_numeric(fallback["energy_mwh"]).fillna(0)
        fallback = fallback[
            fallback["market_stage"].isin(["energy_block", "day_ahead", "real_time"])
            & fallback["energy_mwh"].gt(1e-9)
        ].copy()
        overall = summarize(fallback, "全部", "全部有效交易日回退样本")
        if overall is not None:
            rows.append(overall)
        for period in ["峰", "平", "谷"]:
            r = summarize(fallback, period, "全部有效交易日回退样本")
            if r is not None:
                rows.append(r)
...
def _normal_purchase_allocation(row: pd.Series, purchase_structure: pd.DataFrame) -> tuple[dict, float, str]:
    """按历史正常运行购电结构分配无停机反事实的剩余补购量。"""
    period = str(row.get("时段", ""))
    shares = None
    label = "无历史正常结构"
    if purchase_structure is not None and not purchase_structure.empty:
        part = purchase_structure[purchase_structure["时段"].eq(period)]
        if part.empty:
            part = purchase_structure[purchase_structure["时段"].eq("全部")]
        if not part.empty:
            r = part.iloc[0]
            shares = {
                "能量块": float(r["能量块占比"]),
            ...
            label = str(r.get("样本口径", "历史正常运行结构"))

    if shares is None:
        return {"能量块": 0.0, "日前": 0.0, "实时": 0.0}, np.nan, label

    price_map = {
        "能量块": row.get("能量块加权均价", np.nan),
        "日前": row.get("日前加权均价", np.nan),
        "实时": row.get("实时加权均价", np.nan),
    }
    valid = {m: w for m, w in shares.items() if w > 1e-12 and pd.notna(price_map[m]) and float(price_map[m]) > 0}
    weight_sum = sum(valid.values())
    if weight_sum <= 1e-12:
        # 仅价格缺失时回退LT价格，避免凭空假设市场比例。
        lt_price = row.get("LT加权均价", np.nan)
        return shares, (float(lt_price) if pd.notna(lt_price) and float(lt_price) > 0 else np.nan), label + "；价格缺失回退LT"

    effective = {m: (valid.get(m, 0.0) / weight_sum) for m in shares}
    blended = sum(effective[m] * float(price_map[m]) for m in effective if effective[m] > 0)
    return effective, float(blended), label


def calculate_rt_correction_reference(
    analysis_detail: pd.DataFrame,
    shutdown_period_summary: pd.DataFrame,
    params: dict,
) -> pd.DataFrame:
    """用非停机、非明显负荷下降样本估计正常实时调整率，并生成自适应关注阈值。"""
    cols = ["时段", "样本数", "实时调整率中位数", "实时调整率P90", "采用阈值"]
    if analysis_detail is None or analysis_detail.empty:
        return pd.DataFrame(columns=cols)

    out = analysis_detail.copy()
    out["日期"] = pd.to_datetime(out["日期"], errors="coerce").dt.normalize()
    shutdown_keys = set()
    if shutdown_period_summary is not None and not shutdown_period_summary.empty:
        shutdown_keys = set(
            (pd.Timestamp(r["日期"]).normalize(), str(r["时段"]))
    ...
    # 参考样本进一步排除已经呈现明显负荷下降的时段，避免异常日把“正常RT比例”抬高。
    if "负荷下降信号率" in out.columns:
        out = out[
            out["负荷下降信号率"].isna()
            | out["负荷下降信号率"].lt(params["resale_general_max"])
        ].copy()

    pre = safe_numeric(out["日前后已购电量_MWh"]).abs()
    rt = safe_numeric(out["实时调整_MWh"]).abs()
    out["rt_ratio"] = np.where(pre > 1e-9, rt / pre, np.nan)
    out = out[np.isfinite(out["rt_ratio"])].copy()

    def summarize(g, label):
        s = safe_numeric(g["rt_ratio"]).dropna()
        if s.empty:
            return None
        med = float(s.median())
        p90 = float(s.quantile(0.90)) if len(s) >= 3 else np.nan
        # 历史P90为主；样本不足时用10%。同时设8%下限、15%上限，避免样本污染导致阈值过松。
        raw = p90 if pd.notna(p90) else 0.10
        threshold = min(max(float(raw), 0.08), 0.15)
        return {
            "时段": label,
            "样本数": int(len(s)),
            "实时调整率中位数": med,
    ...
    rows = []
    all_row = summarize(out, "全部")
    if all_row:
        rows.append(all_row)
    for p in ["峰", "平", "谷"]:
        row = summarize(out[out["时段"].eq(p)], p)
        if row:
            rows.append(row)
    return pd.DataFrame(rows, columns=cols)


def build_load_forecast_deviation_events(
    analysis_detail: pd.DataFrame,
    shutdown_period_summary: pd.DataFrame,
    rt_reference: pd.DataFrame,
) -> pd.DataFrame:
    """
    识别非停机情况下的明显实时纠偏。

    主要口径改为 |实时调整| / 日前后已购电量，而不是固定25% LT；阈值优先采用历史正常样本P90。
    同时输出时段级和全天级结果，避免同一天峰/平/谷持续同方向纠偏却只被识别出一个时段。
    """
    if analysis_detail is None or analysis_detail.empty:
        return pd.DataFrame()
    ...
    threshold_map = {"全部": 0.10, "峰": 0.10, "平": 0.10, "谷": 0.10}
    if rt_reference is not None and not rt_reference.empty:
        for _, rr in rt_reference.iterrows():
            threshold_map[str(rr["时段"])] = float(rr["采用阈值"])

    rows = []
    period_flags = []
    for _, r in out.iterrows():
        pre_pos = float(r.get("日前后已购电量_MWh", 0) or 0)
        rt = float(r.get("实时调整_MWh", 0) or 0)
        if pre_pos <= 1e-9:
            continue
        ratio = abs(rt) / pre_pos
        threshold = threshold_map.get(str(r.get("时段", "")), threshold_map["全部"])
        is_event = ratio >= threshold
        period_flags.append((r, ratio, threshold, is_event))
        if not is_event:
            continue
        if rt < -1e-9:
            kind = "实时明显卖出"
            reason = (
                "实时阶段明显降低日前后已购电量，说明实际负荷低于日前阶段预计的用电水平；"
                "在无已知停机前提下，优先归为负荷预测偏高/实际负荷偏低后的实时纠偏。"
            )
        elif rt > 1e-9:
            kind = "实时明显买入"
            reason = (
                "实时阶段明显增加日前后已购电量，说明实际负荷高于日前阶段预计的用电水平；"
                "在无已知停机前提下，优先归为负荷预测偏低/实际负荷偏高后的实时纠偏。"
            )
        else:
            continue
        rows.append({
            "事件层级": "时段",
            "日期": r["日期"], "时段": r["时段"], "类型": kind,
            "LT基础量_MWh": r.get("LT基础量_MWh", 0),
            "能量块调整_MWh": r.get("能量块调整_MWh", 0),
            "日前调整_MWh": r.get("日前调整_MWh", 0),
            "日前后已购电量_MWh": pre_pos,
            "实时调整_MWh": rt,
            "实时纠偏率_日前后已购电量": ratio,
            "判定阈值": threshold,
    ...
    # 全天级：峰/平/谷至少两个时段出现同方向明显纠偏，则直接概括为全天系统性偏差。
    if period_flags:
        flag_df = pd.DataFrame([{
            "日期": r["日期"], "时段": r["时段"],
            "日前后已购电量_MWh": float(r.get("日前后已购电量_MWh", 0) or 0),
            "实时调整_MWh": float(r.get("实时调整_MWh", 0) or 0),
            "ratio": ratio, "threshold": threshold, "flag": is_event,
        } for r, ratio, threshold, is_event in period_flags])
        for d, g in flag_df.groupby("日期"):
            sig = g[g["flag"]].copy()
            sell_n = int((sig["实时调整_MWh"] < -1e-9).sum())
            buy_n = int((sig["实时调整_MWh"] > 1e-9).sum())
            direction = None
            if sell_n >= 2:
                direction = "卖出"
            elif buy_n >= 2:
            ...
            if direction is None:
                continue
            same = g[g["实时调整_MWh"].lt(-1e-9) if direction == "卖出" else g["实时调整_MWh"].gt(1e-9)]
            pre_sum = float(same["日前后已购电量_MWh"].sum())
            rt_sum = float(same["实时调整_MWh"].sum())
            daily_ratio = abs(rt_sum) / pre_sum if pre_sum > 1e-9 else np.nan
            periods = "、".join(sig.loc[
                sig["实时调整_MWh"].lt(-1e-9) if direction == "卖出" else sig["实时调整_MWh"].gt(1e-9),
                "时段"
            ].astype(str).tolist())
            kind = "全天持续实时卖出" if direction == "卖出" else "全天持续实时买入"
            reason = (
                f"当日{periods}段均出现同方向明显实时{direction}，不是单一时段波动；"
                + ("更符合当日实际负荷整体低于日前预测。" if direction == "卖出" else "更符合当日实际负荷整体高于日前预测。")
            )
            rows.append({
                "事件层级": "全天", "日期": d, "时段": "全天", "类型": kind,
                "LT基础量_MWh": np.nan, "能量块调整_MWh": np.nan, "日前调整_MWh": np.nan,
                "日前后已购电量_MWh": pre_sum, "实时调整_MWh": rt_sum,
                "实时纠偏率_日前后已购电量": daily_ratio,
                "判定阈值": threshold_map["全部"], "交易路径": f"明显时段：{periods}", "推理原因": reason,
            })
...
def build_shutdown_period_summary(
    shutdown_split: pd.DataFrame,
    analysis_detail: pd.DataFrame,
    purchase_structure: pd.DataFrame,
) -> pd.DataFrame:
    """
    ...
    - 若理论停机负荷仍高于可解释卖出，则剩余正常需求按历史正常运行日的
      能量块 / 日前 / 实时正向购电结构分配，并用对应市场当日该时段价格估算；
    ...
    alloc = merged.apply(
        lambda r: _normal_purchase_allocation(r, purchase_structure), axis=1
    )
    merged["无停机补购_能量块占比"] = [x[0].get("能量块", 0.0) for x in alloc]
    merged["无停机补购_日前占比"] = [x[0].get("日前", 0.0) for x in alloc]
    merged["无停机补购_实时占比"] = [x[0].get("实时", 0.0) for x in alloc]
    merged["无停机剩余补购参考价_元每MWh"] = [x[1] for x in alloc]
    merged["无停机补购参考口径"] = [x[2] for x in alloc]
    ...
    merged["无停机补购_能量块_MWh"] = merged["无停机额外补购电量_MWh"] * merged["无停机补购_能量块占比"]
    merged["无停机补购_日前_MWh"] = merged["无停机额外补购电量_MWh"] * merged["无停机补购_日前占比"]
    merged["无停机补购_实时_MWh"] = merged["无停机额外补购电量_MWh"] * merged["无停机补购_实时占比"]
...
def build_strategy_review_events(df: pd.DataFrame, params: dict) -> pd.DataFrame:
    """
    事后交易时点复盘只聚焦“能量块买入 vs 后续日前买入”。

    若能量块买入后日前转为净卖出，优先说明负荷/生产信息在能量块之后发生下修，
    不作为“提前买贵了”的价格策略事件；实时市场仅用于最终偏差修正，不再作为提前采购价格好坏的主要比较基准。
    """
    if df.empty:
        return pd.DataFrame()
    ...
    for (d, period), g in out.groupby(["display_date", "峰平谷"]):
        ...
        eb_buy = g[(g["market_stage"].eq("energy_block")) & (g["energy_mwh"] > 1e-9)].copy()
        if eb_buy.empty:
            continue
        eb_buy_energy = float(eb_buy["energy_mwh"].sum())
        eb_price = float(
            (eb_buy["energy_mwh"] * eb_buy["energy_price_yuan_mwh"]).sum() / eb_buy_energy
        )

        da_all = g[g["market_stage"].eq("day_ahead")].copy()
        da_net = float(da_all["energy_mwh"].sum())
        # 日前已经净卖出，说明交易方向发生了负荷下修，不拿它作为“EB提前买贵”的价格策略证据。
        if da_net <= 1e-9:
            continue
        da_buy = da_all[da_all["energy_mwh"] > 1e-9].copy()
        if da_buy.empty:
            continue
        da_buy_energy = float(da_buy["energy_mwh"].sum())
        da_price = float(
            (da_buy["energy_mwh"] * da_buy["energy_price_yuan_mwh"]).sum() / da_buy_energy
        )

        gap = eb_price - da_price
        comparable_energy = min(eb_buy_energy, da_buy_energy)
        ratio = comparable_energy / lt if lt > 1e-9 else np.nan
        impact = max(gap, 0.0) * comparable_energy
        ...
        if (
            ...
            rows.append({
                ...
                "日类型": get_day_type(d),
                "时段": period,
                "复盘类型": "能量块提前买入价格高于后续日前",
                "前序市场": "能量块",
                "后续市场": "日前",
                "能量块买入量_MWh": eb_buy_energy,
                "日前净调整_MWh": da_net,
                "日前买入量_MWh": da_buy_energy,
                "可比较电量_MWh": comparable_energy,
                "占LT基础量": ratio,
                "前序买入均价": eb_price,
                "后续市场参考均价": da_price,
                "价差_元每MWh": gap,
                "潜在影响_元": impact,
                "比较说明": "能量块买入价高于后续日前买入价；仅作事后交易时点复盘。",
            })
...
with tab_analysis:
   ...
   if pd.isna(analysis_end_date) or analysis_filtered.empty:
       ...
       if not (
           ...
           normal_purchase_structure = pd.DataFrame()
           load_forecast_events = pd.DataFrame()
           ...
           # 无论是否存在停机记录，都先基于正常样本统计EB/DA/RT正向购电结构。
           normal_purchase_structure = calculate_normal_purchase_structure(
               analysis_filtered,
               analysis_detail,
               shutdown_split,
               analysis_params,
           ...
           # 非停机情况下，前序市场与实时市场出现大规模反向调整，优先识别为负荷预测偏差/实时纠偏。
           rt_correction_reference = calculate_rt_correction_reference(
               analysis_detail,
               shutdown_period_summary,
               analysis_params,
           )
           load_forecast_events = build_load_forecast_deviation_events(
               analysis_detail,
               shutdown_period_summary,
               rt_correction_reference,
           )

           # -----------------------------------------------------
           # 2) 策略事件：先识别，再排除已知停机和明显负荷预测纠偏时段
           # -----------------------------------------------------
           ...
           if not load_forecast_events.empty:
               for _, r in load_forecast_events.iterrows():
                   d_key = pd.Timestamp(r["日期"]).normalize()
                   if str(r.get("事件层级", "时段")) == "全天":
                       excluded_strategy_keys |= {(d_key, p) for p in ["峰", "平", "谷"]}
                   else:
                       excluded_strategy_keys.add((d_key, str(r["时段"])))

           # 其余明显负荷下降也不直接评价价格策略。
           if not analysis_detail.empty:
               excluded_strategy_keys |= set(
                   (pd.Timestamp(r["日期"]).normalize(), str(r["时段"]))
           ...
           if not strategy_events.empty and excluded_strategy_keys:
               strategy_events = strategy_events[
                   ~strategy_events.apply(
                       lambda r: (pd.Timestamp(r["日期"]).normalize(), str(r["时段"])) in excluded_strategy_keys,
                       axis=1,
                   )
               ].copy()
           ...
           # =====================================================
           # 总结分析：先回答“发生了什么、哪些天、多少钱、是否持续”
           # =====================================================
           st.markdown("### 总结分析")
           st.markdown(
               "**分析基准：** 以三台纸机完全不停机作为标准化参考情景。"
               "纸机假设正常运行功率：**PM1≈31 MW、PM2≈28 MW、PM3≈23 MW**。"
           )
           st.caption(
               "计划检修每月都会发生，但年协/月协合同曲线按合同周期固定，不能因某一两天已知停机单独扣减，"
               "因此无停机反事实保持实际年协/月协合同曲线不变。反事实仅用于标准化比较，不重跑中长期总量70%–105%约束，"
               "也不重跑日前阶段预计用电量95%–105%的交易电量要求；同时使用事后已知的实际市场价格，因此理想化购电成本通常会偏低，"
               "不代表当时真实可实现的最优交易结果。"
           )

           if not normal_purchase_structure.empty:
               st.markdown("**无停机反事实采用的历史正常购电结构**")
               nps = normal_purchase_structure.copy()
               nps["能量块占比(%)"] = nps["能量块占比"] * 100
               nps["日前占比(%)"] = nps["日前占比"] * 100
               nps["实时占比(%)"] = nps["实时占比"] * 100
               nps_show = nps[[
                   "时段", "样本天数", "能量块占比(%)", "日前占比(%)", "实时占比(%)", "样本口径"
               ]]
               st.dataframe(nps_show.round(1), use_container_width=True, hide_index=True)
               st.caption(
                   "比例按正常运行样本中的正向购电量统计：排除已知停机以及明显负荷下降/实时纠偏时段。"
                   "无停机时先撤销可由停机解释的实际卖出；若恢复正常负荷后仍缺电，再按该峰/平/谷的历史EB/日前/实时购电比例分配补购。"
                   "若某市场当天该时段价格缺失，比例会在有有效价格的市场间重新归一；仅在全部后续市场价格都缺失时回退LT价格。"
               )
           ...
           if not shutdown_period_summary.empty:
               ...
               summary_lines.append(
                   ...
                   f"按所有实际停机统一构建无停机标准化反事实，停机对剩余用电均价的金额尺度影响合计约 "
           ...
           # 非停机实时大幅纠偏：总结中列出所有被识别的日期，避免全天事件存在时漏掉其他单时段异常日期。
           if not load_forecast_events.empty:
               lf_daily = load_forecast_events[load_forecast_events["事件层级"].eq("全天")].copy()
               lf_period = load_forecast_events[load_forecast_events["事件层级"].eq("时段")].copy()

               daily_dates = set(pd.to_datetime(lf_daily["日期"]).dt.normalize()) if not lf_daily.empty else set()
               labels = []

               if not lf_daily.empty:
                   for _, r in lf_daily.sort_values("日期").iterrows():
                       forecast_direction_text = (
                           "负荷预测偏高、实际负荷偏低"
                           if float(r.get("实时调整_MWh", 0) or 0) < 0
                           else "负荷预测偏低、实际负荷偏高"
                       )
                       labels.append(
                           f"{pd.Timestamp(r['日期']):%m月%d日}：{forecast_direction_text}，"
                           f"{r['类型']}约{r['实时调整_MWh']:+.1f} MWh"
                       )

               if not lf_period.empty:
                   remaining = lf_period[
                       ~pd.to_datetime(lf_period["日期"]).dt.normalize().isin(daily_dates)
                   ].copy()
                   for d, g in remaining.groupby("日期", sort=True):
                       sell = g[g["实时调整_MWh"] < -1e-9]
                       buy = g[g["实时调整_MWh"] > 1e-9]
                       if len(sell) >= len(buy) and not sell.empty:
                           gg = sell
                           forecast_direction_text = "负荷预测偏高、实际负荷偏低"
                           action_text = "实时卖出"
                       else:
                           gg = buy
                           forecast_direction_text = "负荷预测偏低、实际负荷偏高"
                           action_text = "实时买入"
                       periods = "、".join(gg["时段"].astype(str).tolist())
                       rt_sum = float(gg["实时调整_MWh"].sum())
                       labels.append(
                           f"{pd.Timestamp(d):%m月%d日}：{forecast_direction_text}，"
                           f"{periods}段出现明显{action_text}，合计约{rt_sum:+.1f} MWh"
                       )

               if labels:
                   summary_lines.append(
                       f"排除已知停机后，共识别到 **{len(labels)} 个交易日**存在明显实时纠偏，"
                       "优先判断为**负荷预测与实际用电偏离**。具体日期为：**"
                       + "；".join(labels) + "**。"
                   )

           # 按日期归因汇总：让管理层一眼看到哪些天是停机、预测偏差或价格时点复盘。
           date_cause_parts = []
           if not shutdown_period_summary.empty:
               shutdown_day_labels = []
               for d, g in shutdown_period_summary.groupby("日期", sort=True):
                   machines = "、".join(sorted(set(
                       str(x) for x in g["停机机组"].dropna().astype(str) if str(x).strip()
                   )))
                   shutdown_day_labels.append(
                       f"{pd.Timestamp(d):%m月%d日}（纸机停机{('：' + machines) if machines else ''}）"
                   )
               if shutdown_day_labels:
                   date_cause_parts.append("**生产停机：** " + "、".join(shutdown_day_labels))

           if not load_forecast_events.empty:
               lf_dates_all = sorted(set(pd.to_datetime(load_forecast_events["日期"]).dt.normalize()))
               lf_day_labels = []
               for d in lf_dates_all:
                   gd = load_forecast_events[
                       pd.to_datetime(load_forecast_events["日期"]).dt.normalize().eq(d)
                   ]
                   gd_daily = gd[gd["事件层级"].eq("全天")]
                   rt_total = float(gd_daily["实时调整_MWh"].sum()) if not gd_daily.empty else 0.0
                   if abs(rt_total) <= 1e-9:
                       rt_total = float(gd.loc[gd["事件层级"].eq("时段"), "实时调整_MWh"].sum())
                   cause = "预测偏高" if rt_total < 0 else "预测偏低"
                   lf_day_labels.append(f"{pd.Timestamp(d):%m月%d日}（负荷{cause}）")
               if lf_day_labels:
                   date_cause_parts.append("**负荷预测偏差：** " + "、".join(lf_day_labels))

           if not strategy_events.empty:
               strategy_day_labels = [
                   f"{pd.Timestamp(d):%m月%d日}"
                   for d in sorted(set(pd.to_datetime(strategy_events["日期"]).dt.normalize()))
               ]
               if strategy_day_labels:
                   date_cause_parts.append(
                       "**价格交易时点值得复盘：** " + "、".join(strategy_day_labels)
                   )

           if date_cause_parts:
               summary_lines.append(
                   "**按日期归因汇总：** " + "；".join(date_cause_parts) + "。"
                   "同一天如同时存在停机记录，生产因素优先于负荷预测或价格策略解释。"
               )
           ...
           if not persistent_rows.empty:
               ...
               summary_lines.append(
                   f"剔除已知停机、明显负荷纠偏及小电量/小金额结果后，能量块与日前价格比较出现**持续性不利结果**："
               ...
               summary_lines.append(
                   f"剔除已知停机/明显负荷下降/实时纠偏后，共有 **{len(strategy_events)} 个**“能量块买入 vs 日前买入”结果达到复盘阈值，"
                   "但当前未形成持续性模式，因此归为**偶发不利结果**，不足以判断存在系统性策略问题。"
               )
           else:
               summary_lines.append(
                   "剔除已知停机、明显负荷下降、实时纠偏和小电量/小金额波动后，当前没有识别到需要持续性警告的能量块 vs 日前交易时点问题。"
               )

           if not strategy_events.empty:
               top_se = strategy_events.sort_values("潜在影响_元", ascending=False).head(3)
               issue_labels = [
                   f"{pd.Timestamp(r['日期']):%m月%d日}{r['时段']}段：能量块买入 {r['前序买入均价']:.1f} → 日前买入 {r['后续市场参考均价']:.1f} 元/MWh（能量块贵 {r['价差_元每MWh']:.1f}）"
                   for _, r in top_se.iterrows()
               ]
               summary_lines.append(
                   "价格交易决策复盘可直接回查的重点日期/时段：**" + "；".join(issue_labels) + "**。"
               )
           ...
           st.caption(
               "PM典型运行负荷默认：PM1≈31 MW、PM2≈28 MW、PM3≈23 MW；停机Excel可用load_mw_override覆盖单次记录。"
               "本页统一按实际发生的停机构建‘完全不停机’标准化参考，不再区分计划/非计划类型：即使计划检修提前已知，"
               "年协/月协合同曲线也按合同周期固定，不能因个别停机日单独扣减。"
               "停机区间按15分钟交易段逐段计算真实重叠分钟，并自动归入峰/平/谷。"
               "当前未单独建模停机前降负荷和重启后的爬坡过程，因此stop_end建议填写至纸机大致恢复正常生产负荷的时点；"
               "启动爬坡及实际运行功率变化属于本估算边界。"
           )
           ...
           if shutdown_period_impact.empty:
               ...
               st.caption(
                   "“无停机估算均价”对所有实际停机统一处理：先撤销可由停机解释的实际回售；若恢复正常负荷后仍有缺口，"
                   "再按上方历史正常运行日的EB/日前/实时购电比例分配补购，并使用对应市场当天该时段价格估算。"
                   "年协/月协合同曲线保持实际值不变。该反事实未完整模拟70%–105%中长期总量和日前95%–105%交易电量要求，"
                   "因此属于偏理想化的标准化参考，不替代正式结算。"
               )
           ...
           # 非停机负荷预测偏差 / 实时纠偏
           # =====================================================
           st.markdown("### 非停机负荷预测偏差 / 实时纠偏")
           st.caption(
               "实时市场主要承担最终偏差修正，因此这里不再用固定25%×LT判断。"
               "改为观察｜实时净调整｜÷日前后已购电量，并用历史正常运行样本的实时调整率P90作为主要参考阈值；"
               "为避免样本污染，采用阈值限制在8%–15%之间。若同一天至少两个峰/平/谷时段出现同方向明显纠偏，"
               "会额外标记为全天系统性负荷预测偏差。"
           )
           if not rt_correction_reference.empty:
               rt_ref_show = rt_correction_reference.copy()
               for c in ["实时调整率中位数", "实时调整率P90", "采用阈值"]:
                   rt_ref_show[c + "(%)"] = rt_ref_show[c] * 100
               rt_ref_show = rt_ref_show[[
                   "时段", "样本数", "实时调整率中位数(%)", "实时调整率P90(%)", "采用阈值(%)"
               ]]
               st.dataframe(rt_ref_show.round(1), use_container_width=True, hide_index=True)

           if load_forecast_events.empty:
               st.success("按历史正常实时调整水平，当前未识别到明显的非停机实时纠偏。")
           else:
               lf = load_forecast_events.copy().sort_values(
                   ["事件层级", "实时纠偏率_日前后已购电量"], ascending=[True, False]
               )
               lf["日期"] = pd.to_datetime(lf["日期"]).dt.strftime("%Y-%m-%d")
               lf["实时纠偏率(%)"] = lf["实时纠偏率_日前后已购电量"] * 100
               lf["判定阈值(%)"] = lf["判定阈值"] * 100
               lf_show = lf[[
                   "事件层级", "日期", "时段", "类型", "日前后已购电量_MWh", "实时调整_MWh",
                   "实时纠偏率(%)", "判定阈值(%)", "交易路径", "推理原因"
               ]].rename(columns={
                   "日前后已购电量_MWh": "日前后已购电量(MWh)",
                   "实时调整_MWh": "实时调整(MWh)",
               })
               st.dataframe(lf_show.round(2), use_container_width=True, hide_index=True)
           ...
           st.caption(
               "价格复盘只比较同一日期、同一峰/平/谷内的‘能量块买入价 vs 后续日前买入价’。"
               "实时市场主要用于最终偏差纠正，不再作为判断提前采购价格好坏的主要基准；"
               "若能量块买入后日前已经转为净卖出，则优先视为负荷/生产信息下修，而不列入价格策略事件。"
               "单次达到阈值只称为‘偶发不利结果’，重复、连续或累计影响明显时才触发持续性复盘警告。"
           )
           st.caption(
               "潜在影响按能量块买入量与日前买入量中的较小值作为可比较电量，不再把全部前序买入量都假设可以事后改到更低市场。"
               "实际交易仍受负荷预测、价格预测、中长期总量70%–105%以及日前95%–105%交易电量要求约束，不能仅凭事后价差认定策略失误。"
           )
           ...
           if strategy_events.empty:
               ...
               display_se = se[[
                   "日期", "日类型", "时段", "复盘类型", "前序市场", "后续市场",
                   "能量块买入量_MWh", "日前净调整_MWh", "日前买入量_MWh", "可比较电量_MWh",
                   "占LT基础量(%)", "前序买入均价", "后续市场参考均价",
                   "价差_元每MWh", "潜在影响_元", "比较说明",
               ]].rename(columns={
                   "能量块买入量_MWh": "能量块买入量(MWh)",
                   "日前净调整_MWh": "日前净调整(MWh)",
                   "日前买入量_MWh": "日前买入量(MWh)",
                   "可比较电量_MWh": "可比较电量(MWh)",
                   "前序买入均价": "能量块买入均价(元/MWh)",
                   "后续市场参考均价": "日前买入均价(元/MWh)",
                   "价差_元每MWh": "能量块较日前贵(元/MWh)",
                   "潜在影响_元": "潜在影响(元)",
               })
               st.dataframe(display_se.round(2), use_container_width=True, hide_index=True)

           st.markdown("---")
           st.markdown("### 停机日期 / 时段交易归因（详细）")
           st.caption("以下内容放在本页最后，供需要逐日核查停机交易路径时展开查看。")
           if shutdown_period_summary.empty:
               ...
               for _, r in shutdown_sorted.iterrows():
                   title = (
                       ...
                       f"{r['停机机组']}｜{r.get('负荷下降发现阶段', '交易阶段未知')}"
                   )
                   with st.expander(title, expanded=False):
                       ...
                       extra_buy = float(r.get("无停机额外补购电量_MWh", 0) or 0)
                       if extra_buy > 1e-9:
                           st.caption(
                               f"无停机恢复后仍需额外补购 {extra_buy:,.1f} MWh："
                               f"能量块 {float(r.get('无停机补购_能量块占比', 0) or 0)*100:.1f}%、"
                               f"日前 {float(r.get('无停机补购_日前占比', 0) or 0)*100:.1f}%、"
                               f"实时 {float(r.get('无停机补购_实时占比', 0) or 0)*100:.1f}%；"
                               f"对应混合参考价约 {float(r.get('无停机剩余补购参考价_元每MWh', np.nan)):,.2f} 元/MWh。"
                           )
...
默认PM1=31 MW、PM2=28 MW、PM3=23 MW，可在Excel用`load_mw_override`覆盖。本页不按计划/非计划类型拆分停机影响，只依据实际停机区间恢复“完全不停机”标准化需求。`stop_end`建议填写至大致恢复正常生产负荷的时点；当前未单独模拟停机前降负荷和重启爬坡，因此启动过程与实际功率变化属于估算误差边界。

**3. 停机可解释卖出量**  
取`理论停机负荷下降`与`能量块/日前/实时各阶段实际负调整之和`的较小值。这样既不把超过理论停机负荷的卖出全部归因于停机，也不会因为前序买入抵消实时大卖而漏判。

**4. 无停机反事实均价**  
这里不依赖停机类型标签，只依据“该时段实际发生停机”这一事实做标准化恢复。即使停机提前已知，年协/月协合同曲线也按合同周期固定，不能因为某一两天停机单独扣减，因此LT保持实际曲线不变。先从实际交易结果出发：①把可由停机解释的实际卖出按其真实回售价撤销；②若纸机正常运行后仍缺电，则按历史正常运行样本中该峰/平/谷的能量块、日前、实时正向购电比例分配补购，并使用对应市场当天该时段价格估算；无停机情景提高的是正常用电需求，因此不会人为新增额外卖电，卖电侧只撤销能够由停机解释的真实回售；③得到无停机估算净电量、总成本和综合均价。  
`停机对均价影响 = 实际综合均价 − 无停机估算综合均价`。正值表示停机使剩余用电的单位综合成本上升。该反事实未完整重跑中长期总量70%–105%约束和日前预计用电量95%–105%交易电量要求，又使用了事后已知市场价格，因此估算购电成本通常偏理想化、可能低于真实可实现水平。页面中的“均价影响折算金额”仅用于表达影响尺度，不等同于总电费差额。

**5. 非停机负荷预测偏差**  
在没有已知停机的时段，以`|实时净调整| ÷ 日前后已购电量`衡量实时纠偏强度。阈值优先取历史正常样本P90，并限制在8%–15%之间；因此不再要求达到固定25% LT。若同一天至少两个峰/平/谷时段出现同方向明显实时卖出/买入，会额外归纳为全天系统性负荷预测偏差。

**6. 价格交易决策复盘**  
先排除已知停机、明显负荷下降及上述负荷预测实时纠偏时段。价格复盘只比较`能量块买入价 vs 后续日前买入价`；若能量块买入后日前已经净卖出，则优先说明负荷需求在能量块之后被下修，不作为“提前买贵”的价格策略事件。潜在影响只按能量块买入量与日前买入量中的较小值计算。剩余事件只有在价差≥{strategy_gap:.1f}元/MWh、可比较电量占LT≥{strategy_volume_pct:.1f}%且潜在金额≥{strategy_impact:,.0f}元时才展示。

**7. 重要边界**  
本页是运营与交易复盘工具，不是事后最优交易证明。理想化无停机情景保持实际年协/月协曲线不变：计划检修即使每月都会发生，也不能让售电侧针对某一两天单独修改合同曲线。反事实不完整模拟中长期总量70%–105%及日前预计用电量95%–105%的实际约束，也使用了事后已知价格，因此理想化购电价格通常偏低。真实交易当时还受负荷预测、价格预测和生产信息可得时点影响；所有策略提示均使用“值得复盘 / 可能存在预测偏差”等措辞，不直接认定人为失误。
"""
               )
