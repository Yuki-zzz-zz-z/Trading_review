...
def add_period_shading(fig, selected_day, resolution: str):
    """为现有日内图增加峰/平/谷浅色背景，不改变原图数据和样式。"""
    day = pd.Timestamp(selected_day).normalize()
    points = []
    if resolution == "24小时":
        for h in range(24):
            dt = day + pd.Timedelta(hours=h)
            points.append((f"{h:02d}:00-{h + 1:02d}:00", period_type(dt)))
    else:
        for slot in range(1, 97):
            dt = day + pd.Timedelta(minutes=15 * slot)
            label = "24:00" if slot == 96 else dt.strftime("%H:%M")
    ...
    for i in range(1, len(points) + 1):
        ...
        if changed:
            fig.add_vrect(
                x0=points[start][0], x1=points[i - 1][0],
...
def build_analysis_dataset(df: pd.DataFrame, amount_col: str, params: dict):
    ...
    grouped = (
        ...
        .agg(energy_mwh=("energy_mwh", "sum"), amount_selected=(amount_col, "sum"))
    )
    rows = []
    for (d, period), g in grouped.groupby(["display_date", "峰平谷"]):
        ...
        for stage_name in ["能量块", "日前", "实时"]:
            e = float(stage_energy.get(stage_name, 0.0))
            a = float(stage_amount.get(stage_name, 0.0))
            if e < -1e-9:
                later_sell_energy += -e
                later_sell_amount += -a
        resale_price = later_sell_amount / later_sell_energy if later_sell_energy > 1e-9 else np.nan
        resale_loss = (
            resale_energy * max(lt_price - resale_price, 0.0)
            if resale_energy > 1e-9 and pd.notna(lt_price) and pd.notna(resale_price)
            else 0.0
        )
        rows.append({
            "日期": d, "日类型": get_day_type(d), "时段": period,
            "LT基础量_MWh": lt, "能量块调整_MWh": eb, "日前调整_MWh": da,
            "实时调整_MWh": rt, "后续净调整_MWh": subsequent,
            "基础电量回售_MWh": resale_energy, "基础电量回售率": resale_ratio,
            "回售判断": classify_resale_ratio(resale_ratio, params),
            "LT加权均价": lt_price, "回售等效均价": resale_price,
            "回售价格损失估算_元": resale_loss, "最终净量_MWh": total_energy,
            "最终综合均价": final_price,
        })
...
def build_strategy_watch(df: pd.DataFrame, params: dict):
    ...
    for (d, period), g in out.groupby(["display_date", "峰平谷"]):
        ...
        eb_price = float(eb_buy["signed_amount"].sum() / eb_energy) if eb_energy > 1e-9 else np.nan
        ...
        if later.empty or later_energy_abs <= 1e-9:
            continue
        later_price = float(
            (later["energy_mwh"].abs() * later["energy_price_yuan_mwh"]).sum() / later_energy_abs
        )
        ...
        is_watch = (
            ...
            and (pd.isna(volume_ratio) or volume_ratio >= params["strategy_volume_ratio"])
            and impact >= params["strategy_impact_yuan"]
        )
        if is_watch:
            rows.append({
                "日期": d, "日类型": get_day_type(d), "时段": period,
                "能量块买入量_MWh": eb_energy, "占LT基础量": volume_ratio,
                "能量块买入均价": eb_price, "后续日前实时参考均价": later_price,
                "价差_元每MWh": gap, "潜在影响_元": impact,
            })
    return pd.DataFrame(rows)
...
with tab_analysis:
   ...
   if pd.isna(analysis_end_date) or analysis_filtered.empty:
       ...
       st.caption(
           f"分析期间：{start_ts:%Y-%m-%d} 至 {pd.Timestamp(analysis_end_date):%Y-%m-%d}"
           "（截至当前筛选范围内最新有效实时交易数据）。"
       )

       with st.expander("⚙️ 分析判断参数", expanded=False):
           st.caption("以下参数只影响本Tab的自动判断，不改变现有交易、均价、收益或电费算法。")
           ...
           resale_drop_pct = a3.number_input(
               "疑似重大负荷下降起点（%）", min_value=0.0, max_value=100.0,
           ...
           strategy_gap = b1.number_input(
               "策略关注最小价差（元/MWh）", min_value=0.0,
               value=DEFAULT_ANALYSIS_PARAMS["strategy_price_gap"],
               step=5.0, format="%.1f", key="analysis_strategy_gap"
           )
           strategy_volume_pct = b2.number_input(
               "策略关注最小电量占比（%LT）", min_value=0.0, max_value=100.0,
       ...
       if not (analysis_params["resale_normal_max"] < analysis_params["resale_general_max"] < analysis_params["resale_load_drop_max"]):
           st.error("回售率阈值需满足：正常调整上限 < 一般调整上限 < 疑似重大负荷下降起点。")
       else:
           analysis_period_prices = calculate_period_weighted_prices(analysis_filtered, amount_col)
           analysis_detail = build_analysis_dataset(analysis_filtered, amount_col, analysis_params)
           strategy_watch = build_strategy_watch(analysis_filtered, analysis_params)

           # 交易策略复盘与生产/负荷原因分开：
           # 若某日任一时段已达到“明显负荷下降”及以上（默认回售率>=25%），
           # 则整日不进入策略问题提示，避免将停机/负荷突降误判为交易策略问题。
           load_event_dates = set(
               analysis_detail.loc[
                   analysis_detail["基础电量回售率"].ge(
                       analysis_params["resale_general_max"]
                   ),
                   "日期",
               ].dropna().tolist()
           )
           if not strategy_watch.empty and load_event_dates:
               strategy_watch = strategy_watch[
                   ~strategy_watch["日期"].isin(load_event_dates)
               ].copy()

           peak_p = analysis_period_prices.get("峰", np.nan)
           flat_p = analysis_period_prices.get("平", np.nan)
           valley_p = analysis_period_prices.get("谷", np.nan)
           ...
           if pd.notna(flat_p):
               ...
               if pd.notna(peak_p):
                   diff = flat_p - peak_p
                   comparison_parts.append(f"{'高于' if diff >= 0 else '低于'}峰段 {abs(diff):.2f} 元/MWh")
               if pd.notna(valley_p):
                   diff_v = flat_p - valley_p
                   comparison_parts.append(f"{'高于' if diff_v >= 0 else '低于'}谷段 {abs(diff_v):.2f} 元/MWh")
               suffix = "，" + "；".join(comparison_parts) if comparison_parts else ""
               conclusions.append(
                   f"**平段综合成交加权均价为 {flat_p:.2f} 元/MWh**{suffix}。"
                   "峰平价差本身不直接判定异常，以下结合交易结构解释主要贡献日期。"
               )

           severe = analysis_detail[analysis_detail["基础电量回售率"].ge(analysis_params["resale_load_drop_max"])].copy()
           moderate = analysis_detail[analysis_detail["回售判断"].eq("明显负荷下降")].copy()
           if not severe.empty:
               severe = severe.sort_values("基础电量回售率", ascending=False)
               top_items = [
                   f"{pd.Timestamp(r['日期']):%m月%d日}{r['时段']}段 {r['基础电量回售率'] * 100:.1f}%"
                   for _, r in severe.head(5).iterrows()
               ]
               conclusions.append(
                   "发现**重大负荷下降 / 疑似停机特征**：" + "、".join(top_items) + "。"
                   "这些时段较大比例的年协/月协基础电量通过后续市场净卖出，"
                   "通常对应明显负荷下降或生产停机，建议结合纸机运行记录确认。"
               )
           elif not moderate.empty:
               moderate = moderate.sort_values("基础电量回售率", ascending=False)
               r = moderate.iloc[0]
               conclusions.append(
                   f"本期未出现回售率超过 {analysis_params['resale_load_drop_max']*100:.0f}% 的时段，"
                   f"但 {pd.Timestamp(r['日期']):%m月%d日}{r['时段']}段回售率达到 {r['基础电量回售率']*100:.1f}%，"
                   "表现出明显负荷下降特征。"
               )
           else:
               conclusions.append("本期未识别到明显的大比例基础电量回售特征。")

           resale_loss_total = float(analysis_detail["回售价格损失估算_元"].sum()) if not analysis_detail.empty else 0.0
           resale_loss_rows = analysis_detail[analysis_detail["回售价格损失估算_元"].gt(1e-6)].sort_values("回售价格损失估算_元", ascending=False)
           if not resale_loss_rows.empty:
               r = resale_loss_rows.iloc[0]
               conclusions.append(
                   f"基础电量低价回售的估算价格影响合计约 **{resale_loss_total:,.0f} 元**；"
                   f"其中贡献较大的为 {pd.Timestamp(r['日期']):%m月%d日}{r['时段']}段。"
                   "该估算用于解释交易结构对综合均价的影响，不替代正式结算。"
               )

           if strategy_watch.empty:
               conclusions.append(
                   "按当前价差、电量占比和金额阈值，未识别到需要重点复盘的“能量块高价买入、后续市场明显更低”情形。"
               )
           else:
               strategy_total = float(strategy_watch["潜在影响_元"].sum())
               day_count = strategy_watch["日期"].nunique()
               period_counts = strategy_watch["时段"].value_counts()
               main_period = str(period_counts.index[0]) if not period_counts.empty else ""
               conclusions.append(
                   f"剔开单纯小电量和小价差后，共识别 **{len(strategy_watch)} 个时段 / {day_count} 个交易日**存在策略复盘价值，"
                   f"潜在金额影响约 **{strategy_total:,.0f} 元**，其中以{main_period}段出现次数最多。"
                   "该结果仅提示复盘机会，不直接判定交易策略错误。"
               )

           for i, text in enumerate(conclusions, start=1):
               st.markdown(f"**{i}.** {text}")

           st.markdown("#### 回售率口径与默认判断")
           st.caption(
               "基础电量回售率：某日某峰/平/谷时段内，年协/月协基础电量中，通过后续能量块、日前和实时市场净卖出的比例。"
               "计算：max[-(能量块+日前+实时), 0] ÷ 年协/月协基础电量。采用净调整而非累计卖出，避免中间先卖后买造成重复统计。"
           )
           threshold_df = pd.DataFrame({
               "基础电量回售率": [
                   f"< {resale_normal_pct:.0f}%",
                   f"{resale_normal_pct:.0f}%–{resale_general_pct:.0f}%",
                   f"{resale_general_pct:.0f}%–{resale_drop_pct:.0f}%",
                   f">= {resale_drop_pct:.0f}%",
               ],
               "判断": ["正常调整", "一般调整", "明显负荷下降", "重大负荷下降 / 疑似停机"],
           })
           st.dataframe(threshold_df, use_container_width=True, hide_index=True)

           st.markdown("#### 重点日期与时段诊断")
           if analysis_detail.empty:
               st.info("暂无可诊断数据。")
           else:
               focus = analysis_detail.copy()
               focus["日期"] = pd.to_datetime(focus["日期"]).dt.strftime("%Y-%m-%d")
               focus["回售率"] = focus["基础电量回售率"] * 100
               focus = focus.sort_values(["基础电量回售率", "回售价格损失估算_元"], ascending=[False, False])
               focus_display = focus[[
                   "日期", "日类型", "时段", "LT基础量_MWh", "后续净调整_MWh",
                   "基础电量回售_MWh", "回售率", "回售判断", "LT加权均价",
                   "回售等效均价", "回售价格损失估算_元", "最终综合均价"
               ]].rename(columns={
                   "LT基础量_MWh": "年协/月协基础量(MWh)", "后续净调整_MWh": "后续净调整(MWh)",
                   "基础电量回售_MWh": "基础电量回售(MWh)", "回售率": "回售率(%)",
                   "LT加权均价": "年协/月协均价(元/MWh)", "回售等效均价": "回售等效均价(元/MWh)",
                   "回售价格损失估算_元": "回售价格影响估算(元)", "最终综合均价": "最终综合均价(元/MWh)",
               })
               st.dataframe(focus_display.round(2), use_container_width=True, hide_index=True)

           st.markdown("#### 交易策略复盘提示")
           st.caption(
               "仅当“价差 + 电量占比 + 金额影响”同时达到阈值时才提示，用于过滤偶发小价差或小电量交易。"
               "后续市场价格为日前/实时交易价格的绝对电量加权参考值，用于复盘，不代表交易当时可提前确定的价格。"
           )
           if strategy_watch.empty:
               st.success("当前阈值下无需要重点复盘的交易策略提示。")
           else:
               sw = strategy_watch.copy()
               sw["日期"] = pd.to_datetime(sw["日期"]).dt.strftime("%Y-%m-%d")
               sw["占LT基础量(%)"] = sw["占LT基础量"] * 100
               sw = sw.rename(columns={
                   "能量块买入量_MWh": "能量块买入量(MWh)", "能量块买入均价": "能量块买入均价(元/MWh)",
                   "后续日前实时参考均价": "后续日前/实时参考均价(元/MWh)", "价差_元每MWh": "价差(元/MWh)",
                   "潜在影响_元": "潜在金额影响(元)",
               })
               st.dataframe(
                   sw[["日期", "日类型", "时段", "能量块买入量(MWh)", "占LT基础量(%)",
                       "能量块买入均价(元/MWh)", "后续日前/实时参考均价(元/MWh)",
                       "价差(元/MWh)", "潜在金额影响(元)"]].round(2),
