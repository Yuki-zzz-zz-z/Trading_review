...
def build_load_forecast_deviation_events(
    ...
    """
    识别非停机情况下的明显实时反向纠偏。
    前序（能量块+日前）大买、实时大卖 => 实际负荷低于此前预测；
    前序大卖、实时大买 => 实际负荷高于此前预测。
    """
    ...
    if shutdown_keys:
        out = out[~out.apply(
            lambda r: (pd.Timestamp(r["日期"]).normalize(), str(r["时段"])) in shutdown_keys, axis=1
        )].copy()

    rows = []
    for _, r in out.iterrows():
        lt = float(r.get("LT基础量_MWh", 0) or 0)
        if lt <= 1e-9:
            continue
        pre_rt = float(r.get("能量块调整_MWh", 0) or 0) + float(r.get("日前调整_MWh", 0) or 0)
        rt = float(r.get("实时调整_MWh", 0) or 0)
        threshold = params["resale_general_max"] * lt
        if pre_rt >= threshold and rt <= -threshold:
            kind = "前序买入、实时卖出"
            reason = "实际负荷低于日前阶段预测，实时市场出现明显反向卖出，更符合负荷预测偏差/实际负荷偏低后的实时纠偏。"
        elif pre_rt <= -threshold and rt >= threshold:
            kind = "前序卖出、实时买入"
            reason = "实际负荷高于日前阶段预测，实时市场出现明显反向买入，更符合负荷预测偏差/实际负荷偏高后的实时纠偏。"
        else:
            continue
        rows.append({
            "日期": r["日期"], "时段": r["时段"], "类型": kind,
            "LT基础量_MWh": lt, "能量块调整_MWh": r.get("能量块调整_MWh", 0),
            "日前调整_MWh": r.get("日前调整_MWh", 0), "实时调整_MWh": rt,
            "实时纠偏率_LT": abs(rt) / lt, "交易路径": r.get("交易路径", ""), "推理原因": reason,
        })
    return pd.DataFrame(rows)
...
with tab_analysis:
   ...
   if pd.isna(analysis_end_date) or analysis_filtered.empty:
       ...
       if not (
           ...
           # 非停机实时大幅反向纠偏：优先归为负荷预测偏差。
           if not load_forecast_events.empty:
               lf_top = load_forecast_events.sort_values("实时纠偏率_LT", ascending=False).head(3)
               labels = [
                   f"{pd.Timestamp(r['日期']):%m月%d日}{r['时段']}段（{r['类型']}）"
                   for _, r in lf_top.iterrows()
               ]
               summary_lines.append(
                   f"排除已知停机后，识别到 **{len(load_forecast_events)} 个**明显实时反向纠偏时段，"
           ...
           if not strategy_events.empty:
               ...
               issue_labels = [
                   f"{pd.Timestamp(r['日期']):%m月%d日}{r['时段']}段：{r['前序市场']} {r['前序买入均价']:.1f} → 后续 {r['后续市场参考均价']:.1f} 元/MWh"
           ...
           if shutdown_period_impact.empty:
               ...
               st.caption(
                   "“无停机估算均价”不区分计划/非计划停机：先撤销可由停机解释的实际回售；若恢复正常负荷后仍有缺口，"
           ...
           st.caption(
               "正常情况下，能量块和日前承担主要头寸调整，日前阶段通常已按预计总用电量的95%或105%附近形成头寸。"
               "因此在无已知停机的前提下，如果能量块+日前明显买入而实时大量卖出，或前序明显卖出而实时大量买入，"
               "优先解释为负荷预测偏差/实际负荷偏离日前预估，而不是直接归为价格交易决策问题。"
           )
           if load_forecast_events.empty:
               st.success("当前阈值下未识别到明显的非停机实时反向纠偏时段。")
           else:
               lf = load_forecast_events.copy().sort_values("实时纠偏率_LT", ascending=False)
               lf["日期"] = pd.to_datetime(lf["日期"]).dt.strftime("%Y-%m-%d")
               lf["实时纠偏率_LT(%)"] = lf["实时纠偏率_LT"] * 100
               lf_show = lf[[
                   "日期", "时段", "类型", "LT基础量_MWh", "能量块调整_MWh",
                   "日前调整_MWh", "实时调整_MWh", "实时纠偏率_LT(%)", "交易路径", "推理原因"
               ]].rename(columns={
                   "LT基础量_MWh": "LT基础量(MWh)",
                   "能量块调整_MWh": "能量块调整(MWh)",
                   "日前调整_MWh": "日前调整(MWh)",
           ...
           st.caption(
               "一般市场策略复盘只评价已知停机、明显负荷下降以及负荷预测实时纠偏之外的交易结果；shutdown_type只作为事件背景。若已知停机直到较晚市场才反映，"
               "会在上方生产事件归因中单独提示生产—预测—交易协同复盘。单次达到阈值仅称为‘偶发不利结果’，"
               "不会写成交易策略失误；只有同类型结果重复、连续或累计金额明显时才触发持续性复盘警告。"
           )
           st.caption(
               "这些结果是事后价格比较。负荷预测明显偏差的时段已优先单独归类；这里保留的是更适合回查价格判断和交易时点的日期/时段。"
               "实际交易当时仍受负荷预测、价格预测、中长期总量70%–105%以及日前95%–105%头寸要求等约束，不能仅凭事后价差认定策略失误。"
           )
...
**4. 无停机反事实均价**  
这里不区分停机属于计划、非计划还是不确定，只依据“该时段实际发生停机”这一事实做标准化恢复。计划检修即使提前知道，年协/月协合同曲线也按合同周期固定，不能因为某一两天停机单独扣减，因此LT保持实际曲线不变。先从实际交易结果出发：①把可由停机解释的实际卖出按其真实回售价撤销；②若纸机正常运行后仍缺电，则按历史正常运行样本中该峰/平/谷的能量块、日前、实时正向购电比例分配补购，并使用对应市场当天该时段价格估算；无停机情景提高的是正常用电需求，因此不会人为新增额外卖电，卖电侧只撤销能够由停机解释的真实回售；③得到无停机估算净电量、总成本和综合均价。  
...
**5. 非停机负荷预测偏差**  
在没有已知停机的时段，如果能量块+日前已经明显净买入，但实时出现大规模反向卖出，优先解释为实际负荷低于日前预测；反向情况则优先解释为实际负荷高于日前预测。因为正常业务中能量块和日前承担主要头寸调整，日前阶段通常已按预计总用电量95%或105%附近形成头寸，所以这种实时大幅反向交易首先属于负荷预测/实际负荷偏差的复盘对象，不直接归为价格判断问题。

**6. 价格交易决策复盘**  
先排除已知停机、明显负荷下降及上述负荷预测实时纠偏时段。剩余事件只有在价差≥{strategy_gap:.1f}元/MWh、涉及电量占LT≥{strategy_volume_pct:.1f}%且潜在金额≥{strategy_impact:,.0f}元时才展示，并明确列出日期、峰/平/谷、前序市场、成交价、后续参考价和价差，便于回查。单次结果只称为“偶发不利结果”；达到重复次数、连续性/发生比例或累计金额条件后，才提示“持续性策略复盘警告”。
