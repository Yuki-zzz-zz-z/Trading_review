...
def _infer_load_change_stage(lt, eb, da, rt, params):
    """
    根据能量块→日前→实时的净调整路径，推测负荷下降信息大致何时进入交易头寸。
    ...
    if eb_sig and da_sig and rt_small:
        return (
            "较早已知 / 分阶段调整",
            "能量块阶段已开始明显降低持仓，日前继续卖出，而实时仅小幅调整；"
            "说明主要负荷下修信息在实时之前已经进入交易安排，更符合提前获知后为控制风险分阶段调整头寸。",
        )
    ...
    # 能量块阶段尚未明显降低持仓，日前开始完成主要下修。
    if not eb_sig and da_sig and not rt_sig:
        return (
            "能量块后明确 / 日前主要调整",
            "能量块阶段未出现明显卖出，日前阶段开始大幅降低持仓，实时仅剩小幅调整；"
    ...
    if not eb_sig and da_sig and rt_sig:
        return (
            "日前开始下修，实时继续扩大",
            "能量块阶段未明显卖出，日前已经开始降低持仓，但实时仍继续大幅卖出；"
    ...
    if not eb_sig and not da_sig and rt_sig:
        return (
            "临近运行才明显下修",
            "能量块和日前阶段均未出现明显卖出，而实时阶段突然大幅降低持仓；"
    ...
    if eb_sig and rt_small:
        return (
            "能量块阶段已提前反映",
            "能量块阶段已经明显降低持仓，后续尤其是实时调整较小，说明负荷下降较早已进入交易安排。",
        )
    ...
    if rt_sig:
        return (
            "实时阶段主要下修",
            "主要负调整集中在实时阶段，说明负荷变化较晚才明显反映到交易头寸。",
        )
...
def calculate_rt_correction_reference(
    ...
    pre = safe_numeric(out["日前后持仓_MWh"]).abs()
...
def build_load_forecast_deviation_events(
    ...
    主要口径改为 |实时调整| / 日前后持仓，而不是固定25% LT；阈值优先采用历史正常样本P90。
    ...
    for _, r in out.iterrows():
        pre_pos = float(r.get("日前后持仓_MWh", 0) or 0)
        ...
        if rt < -1e-9:
            kind = "实时明显卖出"
            reason = (
                "实时阶段明显降低日前后持仓，说明实际负荷低于日前阶段形成头寸时的预计水平；"
                "在无已知停机前提下，优先归为负荷预测偏高/实际负荷偏低后的实时纠偏。"
            )
        elif rt > 1e-9:
            kind = "实时明显买入"
            reason = (
                "实时阶段明显增加日前后持仓，说明实际负荷高于日前阶段形成头寸时的预计水平；"
    ...
    if period_flags:
        flag_df = pd.DataFrame([{
            "日期": r["日期"], "时段": r["时段"],
            "日前后持仓_MWh": float(r.get("日前后持仓_MWh", 0) or 0),
        ...
        for d, g in flag_df.groupby("日期"):
            ...
            pre_sum = float(same["日前后持仓_MWh"].sum())
            ...
            rows.append({
                ...
                "日前后持仓_MWh": pre_sum, "实时调整_MWh": rt_sum,
                "实时纠偏率_日前后持仓": daily_ratio,
   ...
   if pd.isna(analysis_end_date) or analysis_filtered.empty:
       ...
       if not (
           ...
           st.caption(
               ...
               "也不重跑日前阶段预计用电量95%–105%的头寸要求；同时使用事后已知的实际市场价格，因此理想化购电成本通常会偏低，"
               "不代表当时真实可实现的最优交易结果。"
           )
           ...
           if not load_forecast_events.empty:
               ...
               if not lf_daily.empty:
                   labels = [
                       f"{pd.Timestamp(r['日期']):%m月%d日}（{r['类型']}，实时调整约{r['实时调整_MWh']:+.1f} MWh）"
                       for _, r in lf_daily.sort_values("实时纠偏率_日前后持仓", ascending=False).head(3).iterrows()
                   ]
                   summary_lines.append(
                       f"排除已知停机后，识别到 **{lf_daily['日期'].nunique()} 个交易日**存在跨多个峰/平/谷时段的同方向实时纠偏，"
                       "更符合整日负荷预测与实际负荷系统性偏离；重点包括 **" + "、".join(labels) + "**。"
                   )
               elif not lf_period.empty:
                   labels = [
                       f"{pd.Timestamp(r['日期']):%m月%d日}{r['时段']}段（{r['类型']}）"
                       for _, r in lf_period.sort_values("实时纠偏率_日前后持仓", ascending=False).head(3).iterrows()
                   ]
                   summary_lines.append(
                       f"排除已知停机后，识别到 **{len(lf_period)} 个**明显实时纠偏时段，"
                       "优先解释为负荷预测偏差/实际负荷与日前预估不一致；重点包括 **"
                       + "、".join(labels) + "**。"
                   )
           ...
           if shutdown_period_impact.empty:
               ...
               st.caption(
                   ...
                   "年协/月协合同曲线保持实际值不变。该反事实未完整模拟70%–105%中长期总量和日前95%–105%头寸约束，"
           ...
           st.caption(
               ...
               "改为观察｜实时净调整｜÷日前后持仓，并用历史正常运行样本的实时调整率P90作为主要参考阈值；"
           ...
           if load_forecast_events.empty:
               ...
               lf = load_forecast_events.copy().sort_values(
                   ["事件层级", "实时纠偏率_日前后持仓"], ascending=[True, False]
               )
               lf["日期"] = pd.to_datetime(lf["日期"]).dt.strftime("%Y-%m-%d")
               lf["实时纠偏率(%)"] = lf["实时纠偏率_日前后持仓"] * 100
               lf["判定阈值(%)"] = lf["判定阈值"] * 100
               lf_show = lf[[
                   "事件层级", "日期", "时段", "类型", "日前后持仓_MWh", "实时调整_MWh",
...
`停机对均价影响 = 实际综合均价 − 无停机估算综合均价`。正值表示停机使剩余用电的单位综合成本上升。该反事实未完整重跑中长期总量70%–105%约束和日前预计用电量95%–105%头寸要求，又使用了事后已知市场价格，因此估算购电成本通常偏理想化、可能低于真实可实现水平。页面中的“均价影响折算金额”仅用于表达影响尺度，不等同于总电费差额。

**5. 非停机负荷预测偏差**  
在没有已知停机的时段，以`|实时净调整| ÷ 日前后持仓`衡量实时纠偏强度。阈值优先取历史正常样本P90，并限制在8%–15%之间；因此不再要求达到固定25% LT。若同一天至少两个峰/平/谷时段出现同方向明显实时卖出/买入，会额外归纳为全天系统性负荷预测偏差。
