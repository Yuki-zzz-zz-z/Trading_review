...
with tab_analysis:
   ...
   if pd.isna(analysis_end_date) or analysis_filtered.empty:
       ...
       if not (
           ...
           analysis_period_prices = calculate_period_weighted_prices(
               analysis_filtered,
               amount_col,
           )
           ...
           strategy_watch = build_strategy_watch(
               analysis_filtered,
               analysis_params,
           )

           # 交易策略复盘与生产/负荷原因分开。
           # 与旧版不同：这里只排除“同一天+同一峰平谷时段”的明显负荷下降信号，
           # 不再因为某日一个时段异常就把整天所有策略复盘都删除。
           if not analysis_detail.empty and not strategy_watch.empty:
               load_event_keys = set(
                   zip(
                       analysis_detail.loc[
               ...
               if load_event_keys:
                   strategy_watch = strategy_watch[
                       ~strategy_watch.apply(
                           lambda r: (r["日期"], r["时段"]) in load_event_keys,
                           axis=1,
                       )
                   ].copy()
           ...
           if pd.notna(flat_p):
               ...
               if pd.notna(peak_p):
                   ...
                   comparison_parts.append(
                       f"{'高于' if diff >= 0 else '低于'}峰段 {abs(diff):.2f} 元/MWh"
                   )
               if pd.notna(valley_p):
                   diff_v = flat_p - valley_p
                   comparison_parts.append(
                       f"{'高于' if diff_v >= 0 else '低于'}谷段 {abs(diff_v):.2f} 元/MWh"
                   )
               suffix = "，" + "；".join(comparison_parts) if comparison_parts else ""
               conclusions.append(
                   f"**平段综合成交加权均价为 {flat_p:.2f} 元/MWh**{suffix}。"
                   "峰平价差本身不直接判定异常，以下结合具体日期、时段和交易路径解释。"
               )

           # 使用“负荷下降信号率”而非只用最终回售率。
           # 这样前面买、后面突然大量卖出的情况也不会被净额抵消。
           severe = analysis_detail[
               analysis_detail["负荷下降信号率"].ge(
                   analysis_params["resale_load_drop_max"]
               )
           ].copy()
           moderate = analysis_detail[
               analysis_detail["负荷下降信号率"].ge(
                   analysis_params["resale_general_max"]
               )
               & analysis_detail["负荷下降信号率"].lt(
                   analysis_params["resale_load_drop_max"]
               )
           ].copy()

           if not severe.empty:
               severe = severe.sort_values("负荷下降信号率", ascending=False)
               top_items = []
               for _, r in severe.head(5).iterrows():
                   resale_txt = (
                       f"回售率{r['基础电量回售率']*100:.1f}%"
                       if pd.notna(r["基础电量回售率"])
                       else "回售率无法计算"
                   )
                   top_items.append(
                       f"{pd.Timestamp(r['日期']):%m月%d日}{r['时段']}段"
                       f"（{resale_txt}，{r['负荷下降发现阶段']}）"
                   )
               conclusions.append(
                   "发现**重大负荷下降 / 疑似停机特征**：" + "、".join(top_items) + "。"
                   "判断不仅考虑最终基础电量回售，也考虑能量块、日前、实时各阶段是否出现大幅下修，"
                   "因此可识别‘前面仍买入、到实时才大量卖出’的晚发现型负荷下降。"
               )
           elif not moderate.empty:
               moderate = moderate.sort_values("负荷下降信号率", ascending=False)
               r = moderate.iloc[0]
               conclusions.append(
                   f"本期未出现超过 {analysis_params['resale_load_drop_max']*100:.0f}% 的重大负荷下降信号，"
                   f"但 {pd.Timestamp(r['日期']):%m月%d日}{r['时段']}段的负荷下降信号率达到 "
                   f"{r['负荷下降信号率']*100:.1f}%，主要表现为{r['负荷下降发现阶段']}。"
               )
           else:
               conclusions.append("本期未识别到明显的大比例基础电量回售或阶段性大幅下修信号。")

           resale_loss_total = (
               float(analysis_detail["回售价格损失估算_元"].sum())
               if not analysis_detail.empty
               else 0.0
           )
           resale_loss_rows = analysis_detail[
               analysis_detail["回售价格损失估算_元"].gt(1e-6)
           ].sort_values("回售价格损失估算_元", ascending=False)
           ...
           if not resale_loss_rows.empty:
               ...
               conclusions.append(
                   ...
                   "该估算仅用于解释交易结构对综合均价的影响，不替代正式结算。"
               )

           if strategy_watch.empty:
               conclusions.append(
                   "剔除同日期同峰平谷时段的明显负荷下降信号后，按当前价差、电量占比和金额阈值，"
                   "未识别到需要重点复盘的‘能量块高价买入、后续市场明显更低’情形。"
               )
           else:
               ...
               conclusions.append(
                   f"剔除明显负荷下降时段以及单纯小电量/小价差后，共识别 "
                   f"**{len(strategy_watch)} 个时段 / {day_count} 个交易日**存在策略复盘价值，"
                   f"潜在金额影响约 **{strategy_total:,.0f} 元**，其中以{main_period}段出现次数最多。"
                   "该结果仅提示复盘机会，不直接判定交易策略错误。"
               )

           for i, text_item in enumerate(conclusions, start=1):
               st.markdown(f"**{i}.** {text_item}")

           st.markdown("#### 回售率与负荷下降信号口径")
           st.caption(
               "基础电量回售率：某日某峰/平/谷时段内，年协/月协基础电量中，通过后续能量块、日前和实时市场净卖出的比例。"
               "计算：max[-(能量块+日前+实时), 0] ÷ 年协/月协基础电量。采用净调整而非累计卖出，避免中间先卖后买重复统计。"
           )
           st.caption(
               "同时增加‘负荷下降信号率’：取基础电量回售率与单一交易阶段最大下修量/LT基础量中的较大值。"
               "因此即使能量块或日前先买入、实时再大量卖出，最终净回售被部分抵消，平台仍能识别晚阶段的大幅需求下修。"
           )
           ...
           # =====================================================
           # 逐个异常日期、逐个峰平谷时段给出交易路径和可能原因。
           # =====================================================
           st.markdown("#### 异常日期与时段逐项分析")

           event_rows = analysis_detail[
               analysis_detail["负荷下降信号率"].ge(
                   analysis_params["resale_general_max"]
               )
           ].copy()
           ...
           if event_rows.empty:
               st.success("当前阈值下没有需要逐项解释的明显负荷下降时段。")
           else:
               for _, r in event_rows.iterrows():
                   event_date = pd.Timestamp(r["日期"])
                   signal_pct = (
                       r["负荷下降信号率"] * 100
                       if pd.notna(r["负荷下降信号率"])
                       else np.nan
                   )
                   resale_pct = (
                       r["基础电量回售率"] * 100
                   ...
                   rt_pre_pct = (
                       r["实时阶段下修率_前序"] * 100
                       if pd.notna(r["实时阶段下修率_前序"])
                       else np.nan
                   )

                   title = (
                       f"{event_date:%Y-%m-%d}｜{r['时段']}段｜"
                   ...
                   with st.expander(title, expanded=True):
                       st.markdown(
                           f"**交易路径：** {r['交易路径']}  "
                           f"  \n**阶段净调整：** 能量块 {r['能量块调整_MWh']:+,.1f} MWh；"
                           f"日前 {r['日前调整_MWh']:+,.1f} MWh；"
                           f"实时 {r['实时调整_MWh']:+,.1f} MWh。"
                       )

                       metric_parts = []
                       if pd.notna(resale_pct):
                           metric_parts.append(f"基础电量回售率 {resale_pct:.1f}%")
                       if pd.notna(signal_pct):
                           metric_parts.append(f"负荷下降信号率 {signal_pct:.1f}%")
                       if pd.notna(rt_pre_pct) and r["实时调整_MWh"] < -1e-9:
                           metric_parts.append(f"实时阶段相对进入实时前持仓下修 {rt_pre_pct:.1f}%")
                       if metric_parts:
                           st.markdown("**关键比例：** " + "；".join(metric_parts) + "。")

                       st.markdown(f"**可能原因推测：** {r['可能原因解释']}")

                       # 明确指出“前面买、后面卖”的路径，方便用户定位何时才知道停机。
                       if (
                           r["能量块调整_MWh"] > 1e-9
                           and r["日前调整_MWh"] > 1e-9
                           and r["实时调整_MWh"] < -1e-9
                           ...
                           st.warning(
                               "该时段能量块和日前均为净买入，但实时市场转为大规模净卖出。"
                               "这说明此前仍按较高负荷安排采购，负荷下降很可能直到实时阶段附近才明确体现；"
                               "如果生产记录显示同一时段纸机临时停机/故障，这一交易路径与该原因高度一致。"
                           )
                       elif (
                           r["能量块调整_MWh"] > 1e-9
                           and r["现货调整_MWh"] < -1e-9
                           ...
                           st.warning(
                               "该时段能量块仍为净买入，但日前+实时现货合计转为净卖出，"
                               "说明需求预期是在能量块交易完成后才明显下调。"
                           )

                       if r["回售价格损失估算_元"] > 1e-6:
                           st.caption(
                               f"该时段基础电量低价回售对价格结构的估算影响约 "
                               f"{r['回售价格损失估算_元']:,.0f} 元；"
                               "该值用于解释综合均价变化，不替代正式结算。"
                           )

           st.markdown("#### 重点日期与时段诊断表")
           if analysis_detail.empty:
               ...
               focus["负荷下降信号率_pct"] = focus["负荷下降信号率"] * 100
               ...
               focus_display = focus[[
                   ...
                   "LT基础量_MWh": "年协/月协基础量(MWh)",
                   "能量块调整_MWh": "能量块调整(MWh)",
                   "日前调整_MWh": "日前调整(MWh)",
                   "实时调整_MWh": "实时调整(MWh)",
                   "后续净调整_MWh": "后续净调整(MWh)",
                   "基础电量回售_MWh": "基础电量回售(MWh)",
                   "回售率": "回售率(%)",
                   "负荷下降信号率_pct": "负荷下降信号率(%)",
                   "LT加权均价": "年协/月协均价(元/MWh)",
                   "回售等效均价": "回售等效均价(元/MWh)",
                   "回售价格损失估算_元": "回售价格影响估算(元)",
                   "最终综合均价": "最终综合均价(元/MWh)",
               })
               st.dataframe(
                   focus_display.round(2),
                   use_container_width=True,
                   hide_index=True,
               )
           ...
           st.caption(
               ...
               "当前默认最小异常价差为10元/MWh，可在上方参数中调整。"
               "同日期同峰平谷时段若已出现明显负荷下降信号，会先从策略异常中排除，避免把生产原因误判为交易策略问题。"
           )
           st.caption(
               "后续市场价格为日前/实时交易价格的绝对电量加权参考值，用于事后复盘，不代表交易当时能够提前确定的价格。"
           )
           ...
           if strategy_watch.empty:
               ...
               sw = sw.rename(columns={
                   ...
                   "能量块买入均价": "能量块买入均价(元/MWh)",
                   "后续日前实时参考均价": "后续日前/实时参考均价(元/MWh)",
                   "价差_元每MWh": "价差(元/MWh)",
                   "潜在影响_元": "潜在金额影响(元)",
               })
               st.dataframe(
